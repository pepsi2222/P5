    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            if epoch > -1:
                ...
                labels = np.array(valid_results["labels"].cpu())
                preds = np.array(valid_results["preds"].cpu())
    ...
    def evaluate_epoch(self, epoch):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(self.val_loader)):
                if self.args.distributed:
                    beam_outputs = self.model.module.generate(
                ...
                preds.extend(gen_yes_probs.tolist())
                labels.extend([1.0 if _ == 'yes' else 0.0 if _ == 'no' else None for _ in batch['target_text']])
            labels = torch.tensor(labels)
            preds = torch.tensor(preds)
            epoch_results = {'labels':labels,'preds':preds}
            dist.barrier()
...
def main_worker(gpu, args):
    ...
    if args.train == 'yelp':
        train_task_list = {'traditional': ['5-1']
        }
    else:
        train_task_list = {'traditional': ['5-1']
        }
